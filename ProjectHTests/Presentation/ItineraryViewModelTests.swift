...
final class ItineraryViewModelTests: XCTestCase {
  ...
  func test_didTapMeal_opensEditorPresetToThatSlot() {
    ...
    XCTAssertEqual(sut.editorViewModel?.showsTimeToggle, false)
    XCTAssertEqual(
  ...
  func test_didTapLodging_opensEditorWithDefaultRange() {
    ...
    XCTAssertEqual(
      calendar.dateComponents(
        [.day],
        from: sut.editorViewModel?.startTime ?? .distantPast,
        to: sut.editorViewModel?.endTime ?? .distantPast
      ).day,
      1
    )
  }
  ...
  func test_didTapLodging_prefillsLocationWithDaysDestination() {
    let plan = Fixtures.dayPlan(
      date: calendar.startOfDay(for: Fixtures.moment(day: 15)),
      destination: .airport(Fixtures.airport(city: "Sapporo", code: "CTS", country: "Japan"))
    )
  ...
  func test_didTapLodging_whenLodgingExists_opensEditorForIt() {
    let tripID = UUID()
    let lodging = Fixtures.lodging(title: "다이이치 타키모토칸", tripID: tripID)
    let plan = Fixtures.dayPlan(
      date: calendar.startOfDay(for: Fixtures.moment(day: 15)),
      lodging: lodging
    )
  ...
  func test_didSelectItem_opensEditorForThatItem() {
    let tripID = UUID()
    let item = Fixtures.itineraryItem(
      startTime: Fixtures.moment(day: 15, hour: 13, minute: 12),
      title: "다누키코지 상점가",
      tripID: tripID
    )
    let plan = Fixtures.dayPlan(
  ...
  func test_didTapMeal_whenMealExists_opensEditorForIt() {
    let tripID = UUID()
    let dinner = Fixtures.itineraryItem(
  ...
  func test_didTapHour_whenHourIsTaken_doesNothing() {
    ...
    let plan = Fixtures.dayPlan(
      date: calendar.startOfDay(for: Fixtures.moment(day: 15)),
      items: [.custom(Fixtures.itineraryItem(startTime: Fixtures.moment(day: 15, hour: 13, minute: 12), tripID: tripID))]
    )
    let sut = makeSUT(plans: [plan])
    waitForPlans(sut, count: 1)
  ...
  func test_firstItemHour_isHourOfEarliestItem() {
    ...
    let plan = Fixtures.dayPlan(
      date: calendar.startOfDay(for: Fixtures.moment(day: 15)),
      items: [
        .custom(Fixtures.itineraryItem(startTime: Fixtures.moment(day: 15, hour: 7, minute: 20), tripID: tripID)),
        .custom(Fixtures.itineraryItem(startTime: Fixtures.moment(day: 15, hour: 19), tripID: tripID)),
      ]
    )
    let sut = makeSUT(plans: [plan])
    waitForPlans(sut, count: 1)
  ...
  func test_didTapHour_offersUnscheduledSightsAsChips() {
    ...
    let scheduled = Fixtures.itineraryItem(
  ...
  func test_didSelectItem_sightOpensWithoutTime() {
    ...
    XCTAssertEqual(sut.editorViewModel?.hasTime, false)
  ...
  private func makePlans() -> [DayPlan] {
    [
      Fixtures.dayPlan(date: calendar.startOfDay(for: Fixtures.moment(day: 15))),
      Fixtures.dayPlan(date: calendar.startOfDay(for: Fixtures.moment(day: 16))),
    ]
  }
